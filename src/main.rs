use std::error::Error;
use std::io;
use std::{env::current_dir, ffi:: OsString, collections::BTreeMap, path::PathBuf};
use std::path::Path;
use crossterm::event::{EnableMouseCapture, DisableMouseCapture, Event, self, KeyCode};
...
use tui::{
    ...
    widgets::{Block, Borders, Clear, ListItem, List, Paragraph},
    Frame, text::Spans, style::{Style, Modifier, Color}, 
};
...
impl App {
    ...
    pub fn get_parapp(&mut self) -> Self{
        ...
        parent.node.current_path = self.current.node.current_path.parent().unwrap().to_path_buf();
        parent.node.set_tp();
        parent.node.set_tc();
    ...
    pub fn which_is_selected(self) -> PathBuf{
        ...
        let selected_item = items.get(self.current.state.selected().expect("aaa")).expect("bbb").clone();
...
impl Node{
    ...
    pub fn set_tp(&mut self){
        ...
        for entry in WalkDir::new(self.current_path.parent().unwrap()).max_depth(1).min_depth(1).sort_by_file_name() {
    ...
    pub fn set_tc(&mut self) {
        ...
        for entry in WalkDir::new(self.current_path.clone()).max_depth(1).min_depth(1).sort_by_file_name() {
            let mut child = child.clone();
            let entry = entry.unwrap();
            let path = entry.path();
            for child_entry in WalkDir::new(path).max_depth(1).min_depth(1).sort_by_file_name(){
    ...
    fn set_current_path(&mut self) {
        ...
        let walker = WalkDir::new(current_dir().unwrap()).max_depth(0).sort_by_file_name().into_iter();
        for entry in walker{
            let a = entry.unwrap();
...
pub fn get_content(path:PathBuf) -> Vec<OsString>{
    let walker = WalkDir::new(&path).max_depth(1).min_depth(1).sort_by_file_name().into_iter();
...
fn draw_curr<B>(f: &mut Frame<B>, area: Rect, app: &mut App)
    ...
    B: Backend,
{
    let items: Vec<ListItem> = app.clone().current.node.tc.into_keys().into_iter()
        .map(|i| {
            let lines = vec![Spans::from(i.to_str().unwrap().to_owned())];
            ListItem::new(lines).style(Style::default().fg(Color::White).bg(Color::DarkGray))
        })
    ...
    B: Backend,
{
    ...
    // let mut items :Vec<ListItem> =Vec::new();
    // let items_a = app.clone().current.node.tc.into_values().into_iter();
    // for j in items_a {
    //     items = j.into_iter().map(|i|{
    //         let lines = vec![Spans::from(i.to_str().unwrap().to_owned())];
    //         ListItem::new(lines).style(Style::default().fg(Color::White).bg(Color::DarkGray))
    //     }).collect();
    // };
    // get_content(app.get_child_path());
    // // Create a List from all list items and highlight the currently selected one
    // let items = List::new(items)
    //     .block(Block::default().borders(Borders::ALL).title("Child"))
    //     .highlight_style(
